...
class VisionTransformer(nn.Module):
    ...
    def forward_features(self, x):
        #print("vision transformer_before_forward_features",np.shape(x))
        x = self.patch_embed(x)
        #print("vision transformer_forward_features_patch_embed",np.shape(x))
        cls_token = self.cls_token.expand(x.shape[0], -1, -1)  # stole cls_tokens impl from Phil Wang, thanks
        if self.dist_token is None:
            x = torch.cat((cls_token, x), dim=1)
        else:
            x = torch.cat((cls_token, self.dist_token.expand(x.shape[0], -1, -1), x), dim=1)
        #print("vision transformer_forward_features:X",np.shape(x))
        #print("vision transformer_forward_features:self.pos_embed",np.shape(self.pos_embed))
