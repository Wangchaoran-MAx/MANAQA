...
def get_lafan1_set(bvh_path, actors, window=50, offset=20):
    ...
    for file in tqdm(bvh_files):
        if file.endswith('.bvh'):
            seq_name, subject = ntpath.basename(file[:-6]).split('_')
            b=seq_name+subject
            a.append(b)
            # find phase file
...
def get_train_stats(bvh_folder, train_set, window, offset):
    ...
    xtrain, qtrain, parents, contacts_l, contacts_r = get_lafan1_set(bvh_folder, train_set,
                                                                     window=window,
                                                                     offset=offset)
...
if __name__ == "__main__":
    ...
    X, Q, parents, contacts_l, contacts_r = get_lafan1_set('/hy-tmp/lafan/', ["subject1", "subject2", "subject3", "subject4"], window=50, offset=20)
